...
struct ArrayList* ArrayList_copyOf(const struct ArrayList* p_list) {
  ...
  struct ArrayList* p_new = ArrayList_new(p_list->capacity);

  for(size_t i = 0; i < p_list->size ;i++){
    p_new->elements[i] = p_list->elements[i];
  }

  p_new->size = p_list->size;
...
void ArrayList_prepend(struct ArrayList* p_list, int element) {
  ...
  //We need to move all elements to the right to have a free place at the first element

  for(int i = p_list->size - 1; i >= 0 ; i--){
    p_list->elements[i+1] = p_list->elements[i];
  }
...
void ArrayList_insert(struct ArrayList* p_list, size_t index, int element) {
    ...
    if (index == p_list->size) {
        ArrayList_append(p_list, element);
        return;
    }
    ...
    for (ssize_t i = (ssize_t)p_list->size - 1; i >= (ssize_t)index; i--) {
        p_list->elements[i + 1] = p_list->elements[i];
    }
...
void ArrayList_delete(struct ArrayList* p_list, size_t index) {
  ...
  for (int i = index; i < p_list->size - 1; i++) {
